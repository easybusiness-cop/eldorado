...
let content = fs.readFileSync('src/db/companyDb.ts', 'utf-8');
  ...
  public evaluateAgentPromotion(agentId: string, projectId: string, taskId: string) {
    ...
    if (promoted || oldGrade !== agent.trainingRecord.competencyGrade) {
        ...
        this.addMemory({
            ...
            employeeId: agent.id, // using employeeId for memory model backwards compatibility
            type: 'episodic',
...
content = content.replace('  public addProject(proj: ProjectPortfolio) {', promoCode + '\n  public addProject(proj: ProjectPortfolio) {');
fs.writeFileSync('src/db/companyDb.ts', content, 'utf-8');
